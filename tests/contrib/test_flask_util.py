...
class FlaskOAuth2Tests(unittest2.TestCase):
    ...
    def test_explicit_configuration(self):
        ...
        assert oauth2.client_id == 'id'
        assert oauth2.client_secret == 'secret'
    ...
    def test_app_configuration(self):
        ...
        with mock.patch('oauth2client.clientsecrets.loadfile',
            ...
            assert oauth2.client_id == 'id2'
            assert oauth2.client_secret == 'secret2'
    ...
    def test_create_flow(self):
        with self.app.test_request_context():
            ...
            assert 'google_oauth2_csrf_token' in flask.session
            assert flask.session['google_oauth2_csrf_token'] == \
                state['csrf_token']
            assert flow.client_id == self.oauth2.client_id
            assert flow.client_secret == self.oauth2.client_secret
            assert 'http' in flow.redirect_uri
            assert 'oauth2callback' in flow.redirect_uri
            ...
            assert flow.params['extra_arg'] == 'test'
    ...
    def test_authorize_view(self):
        with self.app.test_client() as client:
            ...
            assert GOOGLE_AUTH_URI in location
            assert self.oauth2.client_secret not in location
            assert self.oauth2.client_id in q['client_id']
            assert flask.session['google_oauth2_csrf_token'] == \
                state['csrf_token']
        ...
        with self.app.test_client() as client:
            ...
            q = urlparse.parse_qs(location.split('?', 1)[1])
            state = json.loads(q['state'][0])
            assert state['return_url'] == '/test'

        with self.app.test_client() as client:
            response = client.get('/oauth2authorize?extra_param=test')
            location = response.headers['Location']
            assert 'extra_param=test' in location
    ...
    def test_callback_view(self):
        ...
        with self.app.test_client() as client:
            with Http2Mock() as http:
                ...
                assert self.oauth2.client_secret in http.body
                assert 'codez' in http.body
                assert self.oauth2.storage.put.called is True

    def test_authorize_callback(self):
        self.oauth2.authorize_callback = mock.Mock()
        self.test_callback_view()
        assert self.oauth2.authorize_callback.called is True
    ...
    def test_callback_view_errors(self):
        ...
        with self.app.test_client() as client:
            ...
            assert 'something' in response.data.decode('utf-8')
    ...
    def test_no_credentials(self):
        with self.app.test_request_context():
            assert bool(self.oauth2.has_credentials()) is False
            assert self.oauth2.credentials is None
            assert self.oauth2.user_id is None
            assert self.oauth2.email is None
            with pytest.raises(ValueError):
                self.oauth2.http()
            assert self.oauth2.storage.get() is None
    ...
    def test_with_credentials(self):
        ...
        with self.app.test_request_context():
            ...
            assert self.oauth2.credentials.access_token == \
                credentials.access_token
            assert self.oauth2.credentials.refresh_token == \
                credentials.refresh_token
            assert self.oauth2.user_id == '123'
            assert self.oauth2.email == 'user@example.com'
            assert self.oauth2.http()
    ...
    def test_with_expired_credentials(self, utcnow):
        ...
        with self.app.test_request_context():
            self.oauth2.storage.put(credentials)
            assert self.oauth2.has_credentials() is True
    ...
    def test_required(self):
        ...
        with self.app.test_client() as client:
            ...
            assert response.status_code == httplib.FOUND
            assert 'oauth2authorize' in response.headers['Location']
            assert 'protected' in response.headers['Location']
    ...
    def test_incremental_auth(self):
        ...
        with self.app.test_client() as client:
            ...
            assert 'one' in response.headers['Location']
            assert response.status_code == httplib.FOUND
        ...
        with self.app.test_client() as client:
            ...
            assert 'two' in response.headers['Location']
            assert response.status_code == httplib.FOUND
            ...
            assert 'include_granted_scopes' in q
    ...
    def test_incremental_auth_exchange(self):
        ...
        with Http2Mock():
            with self.app.test_client() as client:
                ...
                assert credentials.has_scopes(['email', 'one', 'two']) is True
    ...
    def test_refresh(self):
        with self.app.test_request_context():
            with mock.patch('flask.session'):
                ...
                assert self.oauth2.storage.get().access_token == 'new_token'
    ...
    def test_delete(self):
        with self.app.test_request_context():
            ...
            assert 'google_oauth2_credentials' not in flask.session
